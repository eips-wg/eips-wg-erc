...
A **Non-Fungible Token** (NFT) standard used to vest tokens ([ERC-20](https://eips.fyi/20) or otherwise) over a vesting release curve.

The following standard allows for the implementation of a standard API for NFT based contracts that hold and represent the vested and locked properties of any underlying token ([ERC-20](https://eips.fyi/20) or otherwise) that is emitted to the NFT holder. This standard is an extension of the [ERC-721](https://eips.fyi/721) token that provides basic functionality for creating vesting NFTs, claiming the tokens and reading vesting curve properties.
...
Such a standard will not only provide a much-needed [ERC-20](https://eips.fyi/20) token lock standard, but will also enable the creation of secondary marketplaces tailored for semi-liquid SAFTs.
...
7. Allows tools, front-end apps, aggregators, etc. to show a more holistic view of the vesting tokens and the properties available to users.
   ...
   - Such tooling can easily discover compliance through the [ERC-165](https://eips.fyi/165) `supportsInterface(InterfaceID)` check.
...
- The Vesting NFT standard is meant to be fully backwards compatible with any current [ERC-721](https://eips.fyi/721) integrations and marketplaces.
- The Vesting NFT standard also supports [ERC-165](https://eips.fyi/165) interface detection for detecting `EIP-721` compatibility, as well as Vesting NFT compatibility.
...
- When an [ERC-721](https://eips.fyi/721) approval is made on a Vesting NFT, the operator would have the rights to transfer the Vesting NFT to themselves and then claim the vested tokens.
