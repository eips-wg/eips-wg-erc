...
This proposal extends [ERC-721](https://eips.fyi/721) and [ERC-5192](https://eips.fyi/5192) by introducing Resource Description Framework (RDF) triples to Soulbound Tokens' (‘SBTs‘) metadata.



## Motivation

A Soulbound Token represents the commitments, credentials, and affiliations of accounts. RDF is a standard data model developed by the World Wide Web Consortium (‘W3C’) and is used to represent information in a structured format. Semantic SBTs are built on existing [ERC-721](https://eips.fyi/721) and [ERC-5192](https://eips.fyi/5192) standards to include RDF triples in metadata to capture and store the meaning of social metadata as a network of accounts and attributes.
...
- The token **MUST** implement the following interfaces:
    1. [ERC-165](https://eips.fyi/165)’s `ERC165` (`0x01ffc9a7`)
    1. [ERC-721](https://eips.fyi/721)’s `ERC721` (`0x80ac58cd`)
    1. [ERC-721](https://eips.fyi/721)’s `ERC721Metadata` (`0x5b5e139f`)
    1. [ERC-5192](https://eips.fyi/5192)’s `ERC5192` (`0xb45a3c0e`)
...
This proposal is fully backward compatible with [ERC-721](https://eips.fyi/721) and [ERC-5192](https://eips.fyi/5192).
