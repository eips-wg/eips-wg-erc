...
The standard is proposed as the number and variety of NFT projects increases. The current [ERC-721](https://eips.fyi/721) has a
modest metadata
...
Both the [ERC-721](https://eips.fyi/721) and [ERC-1155](https://eips.fyi/1155) provides natural extension point in the metadata JSON
...
The [ERC-1155](https://eips.fyi/1155) standard has a `properties` property in its JSON Metadata Schema to define an arbitrary
set of additional metadata or properties to describe an underlying asset. Defining the metadata fields in this EIP over
this properties preserve the overall shape of the metadata schema of the [ERC-1155](https://eips.fyi/1155), minimizing the
change needed to list and process the proposed properties.

For tokens minted against the [ERC-721](https://eips.fyi/721) standard, only one additional property needs to be added to the
