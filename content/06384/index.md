...
This EIP introduces the `evalEIP712Buffer` function, which takes an [EIP-712](https://eips.fyi/712) buffer and returns a human-readable text description.

## Motivation

The use case of Web3 off-chain signatures intended to be used within on-chain transaction is gaining traction and being used in multiple leading protocols (e.g. OpenSea) and standards [EIP-2612](https://eips.fyi/2612), mainly as it offers a fee-less experience.
...
While [EIP-712](https://eips.fyi/712) originally declared in its title that being ”humanly readable” is one of its goals, it did not live up to its promise eventually and EIP-712 messages are not understandable by an average user.
...
This toy example shows how an [EIP-20](https://eips.fyi/20) contract supporting this EIP implements an EIP-712 support for "transferWithSig" functionality (a non-standard variation on Permit, as the point of this EIP is to allow readability to non-standard EIP-712 buffers).
