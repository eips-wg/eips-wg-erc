...
This standard is an extension of [ERC-721](https://eips.fyi/721). It proposes a minimal interface for a [ERC-721](https://eips.fyi/721) NFT to be "physically backed" and owned by whoever owns the NFT's physical counterpart.
...
The approach also requires that the contract uses an account-bound implementation of [ERC-721](https://eips.fyi/721) (where all [ERC-721](https://eips.fyi/721) functions that transfer must throw, e.g. the "read only NFT registry" implementation referenced in [ERC-721](https://eips.fyi/721)). This ensures that ownership of the physical item is required to initiate transfers and manage ownership of the NFT, through a new function introduced in this interface described below.
...
To aid recognition that an [ERC-721](https://eips.fyi/721) token implements physical binding via this EIP: upon calling [ERC-165](https://eips.fyi/165)’s `function supportsInterface(bytes4 interfaceID) external view returns (bool)` with `interfaceID=0x4901df9f`, a contract implementing this EIP must return true.
...
This proposal is backward compatible with [ERC-721](https://eips.fyi/721) on an API level. As mentioned above, for the token to be physical-backed, the contract must use a account-bound implementation of [ERC-721](https://eips.fyi/721) (all [ERC-721](https://eips.fyi/721) functions that transfer must throw) so that transfers go through the new function introduced here, which requires a chip signature.
