...
This EIP standardizes an interface for smart contracts to hold of [EIP-721](https://eips.fyi/721) and [EIP-1155](https://eips.fyi/1155) tokens on behalf of ENS domains.
...
1. Compliant contracts MUST implement `ERC721TokenReceiver`, as defined in [EIP-721](https://eips.fyi/721).
...
3. `claimTo` MUST check if `msg.sender` is the owner of the ENS node identified by `bytes32 ensNode` (and/or approved by the domain in implementation-specific ways). The compliant contract then MUST make a call to the `safeTransferFrom` function of [EIP-721](https://eips.fyi/712) or [EIP-1155](https://eips.fyi/1155).
