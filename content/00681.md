...
## Simple Summary
A standard way of representing various transactions, especially payment requests in ether and [ERC-20](https://eips.fyi/20) tokens as URLs.
...
For the syntax of ENS_NAME, please consult [ERC-137](https://eips.fyi/137) defining Ethereum Name Service.
...
Requesting payments in [ERC-20](https://eips.fyi/20) tokens involves a request to call the `transfer` function of the token contract with an `address` and a `uint256` typed parameter, containing the *beneficiary address* and the *amount in atomic units*, respectively. For example,
