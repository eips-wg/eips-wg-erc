...
ETH is designed with *transfer-and-call* as the default behavior in a transaction. Unfortunately, [ERC-20](https://eips.fyi/20) is not designed with that pattern in mind and newer standards cannot apply to the token contracts that have already been deployed.
...
Tokens approved to the Universal Token Router can only be spent in transactions directly signed by their owner, and they have clearly visible token transfer behavior, including token types (ETH, [ERC-20](https://eips.fyi/20), [ERC-721](https://eips.fyi/721) or [ERC-1155](https://eips.fyi/1155)), `amountIn`, `amountOutMin`, and `recipient`.

The Universal Token Router contract is deployed using the [EIP-1014](https://eips.fyi/1014) SingletonFactory contract at a single address across all EVM-compatible networks. This enables new token contracts to pre-configure it as a trusted spender, eliminating the need for approval transactions during their interactive usage.
