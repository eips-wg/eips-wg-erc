...
This standard is an extension of [ERC-20](https://eips.fyi/20). It defines new validation functionality to avoid wallet draining: every `transfer` or `approve` will be locked waiting for validation.
...
[ERC-20](https://eips.fyi/20) compliant contracts MAY implement this EIP.
...
This standard is an extension of [ERC-20](https://eips.fyi/20), compatible with all the operations except `transfer`/`transferFrom`/`approve`.
