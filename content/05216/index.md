...
This EIP defines standard functions for granular approval of [EIP-1155](https://eips.fyi/1155) tokens by both `id` and `amount`. This EIP extends [EIP-1155](https://eips.fyi/1155).

## Motivation

[EIP-1155](https://eips.fyi/1155)'s popularity means that multi-token management transactions occur on a daily basis. Although it can be used as a more comprehensive alternative to [EIP-721](https://eips.fyi/721), EIP-1155 is most commonly used as intended: creating multiple `id`s, each with multiple tokens. While many projects interface with these semi-fungible tokens, by far the most common interactions are with NFT marketplaces.

Due to the nature of the blockchain, programming errors or malicious operators can cause permanent loss of funds. It is therefore essential that transactions are as trustless as possible. EIP-1155 uses the `setApprovalForAll` function, which approves ALL tokens with a specific `id`. This system has obvious minimum required trust flaws. This EIP combines ideas from [EIP-20](https://eips.fyi/20) and [EIP-721](https://eips.fyi/721) in order to create a trust mechanism where an owner can allow a third party, such as a marketplace, to approve a limited (instead of unlimited) number of tokens of one `id`.
...
By having a way to approve and revoke in a manner similar to [EIP-20](https://eips.fyi/20), the trust level can be more directly managed by users:
...
The [EIP-20](https://eips.fyi/20) name patterns were used due to similarities with [EIP-20](https://eips.fyi/20) approvals.

## Backwards Compatibility

This standard is compatible with [EIP-1155](https://eips.fyi/1155).

## Reference Implementation

The reference implementation can be found [here](./assets/ERC1155ApprovalByAmount.sol).
