...
It designates a field in which it's called `majorERCIdentifier` which is normally known or referred to as "ERC number". For example, `ERC-721` aka [ERC-721](https://eips.fyi/721) has a `majorERCIdentifier = 721`. This ERC has a `majorERCIdentifier = 5269`.
...
This ERC is created as a competing standard for [ERC-165](https://eips.fyi/165).

Here are the major differences between this ERC and [ERC-165](https://eips.fyi/165).

1. [ERC-165](https://eips.fyi/165) uses the hash of a method's signature which declares the existence of one method or multiple methods,
...
1. [ERC-165](https://eips.fyi/165) doesn't provide query ability based on the caller.
...
- [ERC-5267](https://eips.fyi/5267) specifies `extensions` to be a list of ERC numbers.
- [ERC-600](https://eips.fyi/600), and [ERC-601](https://eips.fyi/601) specify an `ERC` number in the `m / purpose' / subpurpose' / ERC' / wallet'` path.
- [ERC-5568](https://eips.fyi/5568) specifies `The instruction_id of an instruction defined by an ERC MUST be its ERC number unless there are exceptional circumstances (be reasonable)`
- [ERC-6120](https://eips.fyi/6120) specifies `struct Token { uint eip; ..., }` where `uint eip` is an ERC number to identify ERCs.
...
4. Compared to [ERC-165](https://eips.fyi/165), we also add an additional input of `address caller`, given the increasing popularity of proxy patterns such as those enabled by [ERC-1967](https://eips.fyi/1967). One may ask: why not simply use `msg.sender`? This is because we want to allow query them without transaction or a proxy contract to query whether interface ERC-`number` will be available to that particular sender.
...
Here is an example where a contract of [ERC-721](https://eips.fyi/721) also implement this ERC to make it easier
...
Similar to [ERC-165](https://eips.fyi/165) callers of the interface MUST assume the smart contract
