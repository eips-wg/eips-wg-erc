...
This EIP establish a general protocol for permitting approving function calls in the same transaction rely on [ERC-5750](https://eips.fyi/5750).
Unlike a few prior art ([ERC-2612](https://eips.fyi/2612) for [ERC-20](https://eips.fyi/20), `ERC-4494` for [ERC-721](https://eips.fyi/721) that
...
As specified in [ERC-5750 General Extensibility for Method Behaviors](https://eips.fyi/5750), any compliant method that has an `bytes extraData` as its
last method designated for extending behaviors can conform to [ERC-5453](https://eips.fyi/5453) as the way to indicate a permit from certain user.

1. Any compliant method of this EIP MUST be a [ERC-5750](https://eips.fyi/5750) compliant method.
...
8. Each `bytes sig` MUST be an ECDSA (secp256k1) signature using private key of signer whose corresponding address is `endorserAddress` signing `validityDigest` which is the a hashTypeDataV4 of [EIP-712](https://eips.fyi/712) of hashStruct of `ValidityBound` data structure as followed:
...
The design assumes a `bytes calldata extraData` to maximize the flexibility of future extensions. This assumption is compatible with [ERC-721](https://eips.fyi/721), [ERC-1155](https://eips.fyi/1155) and many other ERC-track EIPs. Those that aren't, such as [ERC-20](https://eips.fyi/20), can also be updated to support it, such as using a wrapper contract or proxy upgrade.
