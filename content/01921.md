...
## Simple Summary
In the context of dType, the Decentralized Type System described in [EIP-1900](https://eips.fyi/1900), we are proposing to add support for registering functions (with a preference for `pure` and `view`) in the dType Registry.

## Abstract

This proposal is part of a series of EIPs focused on expanding the concept of a Decentralized Type System, as explained in [EIP-1900](https://eips.fyi/1900).
...
Registering new smart contract functions can be done through the same consensus mechanism as [EIP-1900](https://eips.fyi/1900) mentions, in order to avoid burdening the chain state with redundant or improper records.
