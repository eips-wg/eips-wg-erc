...
1. The relying party generates a SIWE Message and prefixes the SIWE Message with `\x19Ethereum Signed Message:\n<length of message>` as defined in [ERC-191](https://eips.fyi/191).
2. The wallet presents the user with a structured plaintext message or equivalent interface for signing the SIWE Message with the [ERC-191](https://eips.fyi/191) signed data format.
3. The signature is then presented to the relying party, which checks the signature's validity and SIWE Message content.
4. The relying party might further fetch data associated with the Ethereum address, such as from the Ethereum blockchain (e.g., ENS, account balances, [ERC-20](https://eips.fyi/20)/[ERC-721](https://eips.fyi/721)/[ERC-1155](https://eips.fyi/1155) asset ownership), or other data sources that might or might not be permissioned.
...
- `address` REQUIRED. The Ethereum address performing the signing. Its value SHOULD be conformant to mixed-case checksum address encoding specified in [ERC-55](https://eips.fyi/55) where applicable.
...
- `chain-id` REQUIRED. The [EIP-155](https://eips.fyi/155) Chain ID to which the session is bound, and the network where Contract Accounts MUST be resolved.
...
- For Externally Owned Accounts (EOAs), the verification method specified in [ERC-191](https://eips.fyi/191) MUST be used.

- For Contract Accounts,
    - The verification method specified in [ERC-1271](https://eips.fyi/1271) SHOULD be used, and if it is not, the implementer MUST clearly define the verification method to attain security and interoperability for both wallets and relying parties.
    - When performing [ERC-1271](https://eips.fyi/1271) signature verification, the contract performing the verification MUST be resolved from the specified `chain-id`.
    - Implementers SHOULD take into consideration that [ERC-1271](https://eips.fyi/1271) implementations are not required to be pure functions, and can return different results for the same inputs depending on blockchain state. This can affect the security model and session validation rules. For example, a service with [ERC-1271](https://eips.fyi/1271) signing enabled could rely on webhooks to receive notifications when state affecting the results is changed. When it receives a notification, it invalidates any matching sessions.
...
- The relying party or wallet MAY additionally perform resolution of ENS data, as this can improve the user experience by displaying human-friendly information that is related to the `address`. Resolvable ENS data include:
    - The [primary ENS name](https://eips.fyi/181).
    - The ENS avatar.
...
- Wallet implementers SHOULD warn users if the substring `"wants you to sign in
  with your Ethereum account"` appears anywhere in an [ERC-191](https://eips.fyi/191) message signing
...
- The origin SHOULD be read from a trusted data source such as the browser window or over WalletConnect ([ERC-1328](https://eips.fyi/1328)) sessions for comparison against the signing message contents.
...
- Why [ERC-191](https://eips.fyi/191) (Signed Data Standard) over [EIP-712](https://eips.fyi/712) (Ethereum typed structured data hashing and signing)
    - [ERC-191](https://eips.fyi/191) is already broadly supported across wallets UX, while [EIP-712](https://eips.fyi/712) support for friendly user display is pending. **(1, 2, 3, 4)**
    - [ERC-191](https://eips.fyi/191) is simple to implement using a pre-set prefix prior to signing, while [EIP-712](https://eips.fyi/712) is more complex to implement requiring the further implementations of a bespoke Solidity-inspired type system, RLP-based encoding format, and custom keccak-based hashing scheme. **(2)**
    - [ERC-191](https://eips.fyi/191) produces more human-readable messages, while [EIP-712](https://eips.fyi/712) creates signing outputs for machine consumption, with most wallets not displaying the payload to be signed in a manner friendly to humans. **(1)**![](./assets/signing.png)

    - [EIP-712](https://eips.fyi/712) has the advantage of on-chain representation and on-chain verifiability, such as for their use in metatransactions, but this feature is not relevant for the specification's scope. **(2)**
...
- Possible future support for [EIP-712](https://eips.fyi/712).
- Version interpretation rules, e.g., sign with minor revision greater than understood, but not greater major version.

## Backwards Compatibility

- Most wallet implementations already support [ERC-191](https://eips.fyi/191), so this is used as a base pattern with additional features.
...
- Certain wallets can use smart contracts and multisigs to provide an enhanced user experiences with respect to key usage and key recovery, and these can be supported via [ERC-1271](https://eips.fyi/1271) signing.
...
- If an [ERC-1271](https://eips.fyi/1271) implementation or dependent data changes the signature computation, the server SHOULD invalidate sessions appropriately.
