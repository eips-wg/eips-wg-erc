...
* **Support other use cases:**
  ...
  * Atomic multi-operations (similar to [EIP-3074](https://eips.fyi/3074)).
  * Payment of transaction fees using [ERC-20](https://eips.fyi/20) tokens.
...
| feeToken          | address | contract address of the [ERC-20](https://eips.fyi/20) token used to repay the bundler. _(`address(0)` for the native token)_.                                       |
...
| maxFeePerGas      | uint256 | max amount of basefee that the `operation` execution is expected to pay. _(Similar to [EIP-1559](https://eips.fyi/1559) `max_fee_per_gas`)_.                        |
| priorityFeePerGas | uint256 | fixed amount of fees that the `operation` execution is expected to pay to the bundler. _(Similar to [EIP-1559](https://eips.fyi/1559) `max_priority_fee_per_gas`)_. |
...
* The `feeToken` is `address(0)` or a known [ERC-20](https://eips.fyi/20) token that the bundler is willing to accept.
