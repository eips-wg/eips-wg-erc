...
## Abstract
This proposal specifies a JSON schema for describing lists of remote procedure call (RPC) providers for Ethereum-like chains, including their supported [EIP-155](https://eips.fyi/155) `CHAIN_ID`.

## Motivation
The recent explosion of alternate chains, scaling solutions, and other mostly Ethereum-compatible ledgers has brought with it many risks for users. It has become commonplace to blindly add new RPC providers using [EIP-3085](https://eips.fyi/3085) without evaluating their trustworthiness. At best, these RPC providers may be accurate, but track requests; and at worst, they may provide misleading information and frontrun transactions.
...
Provider lists SHOULD be published to an Ethereum Name Service (ENS) name using [EIP-1577](https://eips.fyi/1577)'s `contenthash` mechanism on mainnet.
