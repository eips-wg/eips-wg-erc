...
This standard is an extension of [EIP-721](https://eips.fyi/721). It adds a `MetadataUpdate` event to EIP-721 tokens.

## Motivation

Many [EIP-721](https://eips.fyi/721) contracts emit an event when one of its tokens' metadata are changed. While tracking changes based on these different events is possible, it is an extra effort for third-party platforms, such as an NFT marketplace, to build individualized solutions for each NFT collection.
