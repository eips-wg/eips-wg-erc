...
This extension adds a `decreaseAllowanceBySpender` function to decrease [ERC-20](https://eips.fyi/20) allowances, in which a spender can revoke or decrease a given allowance by a specific address. This ERC extends [ERC-20](https://eips.fyi/20).

## Motivation

Currently, [ERC-20](https://eips.fyi/20) tokens offer allowances, enabling token owners to authorize spenders to use a designated amount of tokens on their behalf. However, the process of decreasing an allowance is limited to the owner's side, which can be problematic if the token owner is a treasury wallet or a multi-signature wallet that has granted an excessive allowance to a spender. In such cases, reducing the allowance from the owner's perspective can be time-consuming and challenging.
...
- The decision to maintain naming patterns similar to [ERC-20](https://eips.fyi/20)'s approvals is rooted in promoting consistency and ease of understanding for developers familiar with [ERC-20](https://eips.fyi/20) standard.

## Backwards Compatibility

This standard is compatible with [ERC-20](https://eips.fyi/20).
