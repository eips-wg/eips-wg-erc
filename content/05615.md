...
This ERC standardizes an existing mechanism to fetch token supply data from [ERC-1155](https://eips.fyi/1155) tokens. It adds a `totalSupply` function, which fetches the number of tokens with a given `id`, and an `exists` function, which checks for the existence of a given `id`.
...
Implementations MAY support [ERC-165](https://eips.fyi/165) interface discovery, but consumers MUST NOT rely on it.

## Rationale

This ERC does not implement [ERC-165](https://eips.fyi/165), as this interface is simple enough that the extra complexity is unnecessary and would cause incompatibilities with pre-existing implementations.

The `totalSupply` and `exists` functions were modeled after [ERC-721](https://eips.fyi/721) and [ERC-20](https://eips.fyi/20).
