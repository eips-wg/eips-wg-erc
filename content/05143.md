...
The following standard extends the [EIP-4626](https://eips.fyi/4626) Tokenized Vault standard with functions dedicated to the safe interaction between EOAs and the vault when price is subject to slippage.

## Motivation

[EIP-4626](https://eips.fyi/4626) security considerations section states that:
...
MUST support [EIP-20](https://eips.fyi/20) `approve` / `transferFrom` on `asset` as a deposit flow.
